var simPanel = new function() {
  ...
  this.init = function() {
    ...
    self.$plotterBtn = $('.plotter');

    setOnClickAnimation([self.$runSim, self.$world, self.$reset, self.$camera, self.$ruler, self.$sensors, self.$joystickIcon, self.$hubButtonsIcon, self.$plotterBtn]);
    ...
    self.$plotterBtn.click(self.togglePlotter);
  ...
  // Toggle plotter window
  this.togglePlotter = function() {
    self.$plotter.toggleClass('hide');
  };
  ...
  this.stopSim = function(stopRobot) {
    ...
    if (self.plotInterval) {
      clearInterval(self.plotInterval);
      self.plotInterval = null;
    }
  ...
  this.runSim = function() {
    if (skulpt.running) {
      ...
      // Check if we're in Arduino mode
      var isArduinoMode = (blockly.generatorType === 'arduino');
      
      if (isArduinoMode && !filesManager.modified) {
        // Arduino mode: Execute JavaScript directly
        console.log('Running Arduino code in JavaScript mode...');
        
        // Generate JavaScript from Arduino blocks
        var jsCode = arduino_generator.genJavaScript();
        ...
        var fullCode = 
          ...
          '  } catch(e) {\n' +
          '    console.error("Arduino execution error:", e);\n' +
          '  }\n' +
          '})();';
        ...
        try {
          eval(fullCode);
          self.plotInterval = setInterval(function(){
            if (typeof window._arduinoPlotTick === 'function') {
              window._arduinoPlotTick();
            }
          }, 200);
          ...
          acknowledgeDialog({
            ...
            message: 'Error running Arduino code: ' + e.message
          });
        }
        
      } else {
        // Python mode or modified code: Use Skulpt
        if (! filesManager.modified) {
          // Generate Python code from blocks
          console.log('Generating Python code from workspace...');
          var pythonCode = Blockly.Python.workspaceToCode(blockly.workspace);
          console.log('Raw workspace Python code:', pythonCode);
          
          // Wrap in ev3dev2 imports and setup
          var fullCode =
            '#!/usr/bin/env python3\n' +
            '\n' +
            ...
            'import time\n' +
            'import math\n' +
            'from ev3dev2.motor import LargeMotor, OUTPUT_A, OUTPUT_B, SpeedPercent, MoveTank, MoveJoystick, MoveSteering\n' +
          ...
          console.log('Final Python code for simulation:', fullCode);
          
          // Ensure main.py exists and has the generated code
          filesManager.setToDefault();
          filesManager.files['main.py'] = fullCode;
        }
