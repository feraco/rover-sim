var main = new function() {
  ...
  this.init = function() {
    ...
    self.$arduinoMenu.click(self.toggleArduinoMenu);
    ...
    // Setup Python/Arduino toggle buttons
    ...
    // Load Arduino code on page load - wait for blockly to initialize
    function loadArduinoCode() {
      if (blockly.generator && blockly.displayedWorkspace) {
        pythonPanel.loadPythonFromBlockly();
      } else {
        setTimeout(loadArduinoCode, 100);
      }
    }
  ...
  this.openAbout = function() {
    let $body = $(
      '<div class="about">' +
        '<p><a href="https://innovatedstem.com" target="_blank">InnovatedSTEM.com</a></p>' +
      '</div>'
    );
  ...
  // Toggle Arduino menu
  this.toggleArduinoMenu = function(e) {
    if ($('.arduinoMenuDropDown').length == 0) {
      $('.menuDropDown').remove();
      e.stopPropagation();

      let menuItems = [
        {html: 'Code Editor', line: true, callback: function() { /* already shown */ }},
  ...
  this.toggleFileMenu = function(e) {
    if ($('.fileMenuDropDown').length == 0) {
      ...
      let menuItems = [
        ...
        {html: 'Download Arduino (.ino)', line: false, callback: self.saveArduinoToComputer},
  ...
  this.newProgram = function() {
    confirmDialog(i18n.get('#main-start_new_warning#'), function() {
      ...
      localStorage.setItem('roverPythonModified', false);
  ...
  this.saveZipToComputer = function() {
    ...
    var zip = new JSZip();
    zip.file('roverBlocks.xml', blockly.getXmlText());
    if (filesManager.modified) {
    ...
    zip.file('roverRobot.json', JSON.stringify(robot.options, null, 2));
        ...
        reader.onload = function(e) {
          JSZip.loadAsync(e.target.result)
            .then(async function(zip) {
              ...
              if ('roverBlocks.xml' in zip.files) {
                const xmlText = await loadFile(zip, 'roverBlocks.xml');
                blockly.loadXmlText(xmlText);
              }

              if ('roverRobot.json' in zip.files) {
                const robotConf = await loadFile(zip, 'roverRobot.json')
  ...
  // save Arduino code to computer
  this.saveArduinoToComputer = function() {
    let filename = self.$projectName.val();
    if (filename.trim() == '') {
      filename = 'roverBot';
    }
    ...
    let arduinoCode = arduino_generator.genCode();
    
    self.downloadFile(filename + '.ino', arduinoCode, 'text/plain');
  };
  ...
  this.loadPythonFromComputerZip = function(e) {
    async function loadFiles(zip) {
      ...
      if (! filenames.includes('main.py') && ! filenames.includes('roverPython.py')) {
        console.log('No main.py or roverPython.py in zip archive');
        throw new Error();
      }
